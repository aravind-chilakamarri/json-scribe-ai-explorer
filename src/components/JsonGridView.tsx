...
function PrimitiveValue({ value, searchQuery }: { value: any; searchQuery: string }) {
  ...
  if (typeof value === 'number') {
    return <span className="text-blue-600 font-mono">{highlightText(String(value), searchQuery)}</span>;
  }
  // string
  return <span className="text-emerald-700">{highlightText(String(value), searchQuery)}</span>;
}
    ...
    return (
      <div className="w-full">
        <button
          type="button"
          onClick={toggle}
          className="inline-flex items-center gap-2 text-blue-700 hover:text-blue-900 font-medium py-1 px-1 -ml-1 rounded hover:bg-blue-50 transition-colors"
        >
          {isExpanded
...
function ObjectGrid({ data, depth }: { data: Record<string, any>; depth: number }) {
  ...
  const borderColor = isRoot ? 'border-slate-300' : 'border-slate-200';
    ...
    <table className={`w-full border-collapse border ${borderColor}`}>
      <tbody>
        {entries.map(([key, value], i) => (
          <tr
            key={key}
            className={`${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'} hover:bg-blue-50/40 transition-colors`}
          >
            <td
              className={`border ${borderColor} px-4 ${isRoot ? 'py-3.5' : 'py-2.5'} font-semibold text-blue-700 whitespace-nowrap align-top w-[1%] ${isRoot ? 'text-base' : 'text-sm'}`}
            >
              {highlightText(key, searchQuery)}
            </td>
            <td className={`border ${borderColor} px-4 ${isRoot ? 'py-3.5' : 'py-2.5'} align-top ${isRoot ? 'text-base' : 'text-sm'}`}>
        ...
        <table className={`w-full border-collapse border ${borderColor}`}>
          <thead>
            <tr className="bg-slate-100 sticky top-0 z-10">
              <th className={`border ${borderColor} px-4 ${isRoot ? 'py-3' : 'py-2.5'} text-left text-xs font-bold text-slate-500 uppercase tracking-wider w-[1%] whitespace-nowrap`}>
                #
              </th>
              {columns.map(col => (
                <th
                  key={col}
                  className={`border ${borderColor} px-4 ${isRoot ? 'py-3' : 'py-2.5'} text-left text-xs font-bold text-slate-700 uppercase tracking-wider whitespace-nowrap`}
                >
                  {col}
                </th>
              ))}
          ...
          <tbody>
            {data.map((item, rowIdx) => {
              if (typeof item === 'object' && item !== null && !Array.isArray(item)) {
                return (
                  <tr key={rowIdx} className={`${rowIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'} hover:bg-blue-50/40 transition-colors`}>
                    <td className={`border ${borderColor} px-4 ${isRoot ? 'py-3' : 'py-2.5'} text-xs text-slate-400 font-mono text-center align-top`}>
                      {rowIdx}
                    </td>
                    {columns.map(col => (
                      <td key={col} className={`border ${borderColor} px-4 ${isRoot ? 'py-3' : 'py-2.5'} align-top ${isRoot ? 'text-base' : 'text-sm'}`}>
              ...
              return (
                <tr key={rowIdx} className={`${rowIdx % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'} hover:bg-blue-50/40`}>
                  <td className={`border ${borderColor} px-4 ${isRoot ? 'py-3' : 'py-2.5'} text-xs text-slate-400 font-mono text-center align-top`}>
                    {rowIdx}
                  </td>
                  <td colSpan={columns.length} className={`border ${borderColor} px-4 ${isRoot ? 'py-3' : 'py-2.5'} align-top`}>
      ...
      <tbody>
        {data.map((item, i) => (
          <tr key={i} className={`${i % 2 === 0 ? 'bg-white' : 'bg-slate-50/60'} hover:bg-blue-50/40 transition-colors`}>
            <td className={`border ${borderColor} px-4 ${isRoot ? 'py-3' : 'py-2.5'} text-xs text-slate-400 font-mono text-center align-top w-[1%] whitespace-nowrap`}>
              {i}
            </td>
            <td className={`border ${borderColor} px-4 ${isRoot ? 'py-3' : 'py-2.5'} align-top ${isRoot ? 'text-base' : 'text-sm'}`}>
              <GridValue value={item} depth={depth} />
            </td>
          </tr>
        ))}
...
export const JsonGridView: React.FC<JsonGridViewProps> = ({
  ...
  if (data === null || data === undefined) {
    return <div className="text-sm text-gray-400 p-6">No data</div>;
  }

  return (
    <GridContext.Provider value={ctx}>
      <div className="rounded-lg border border-slate-300 bg-white shadow-sm overflow-auto h-full">
        {Array.isArray(data) ? (
